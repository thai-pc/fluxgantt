...
5. **Tree-shakable + bundle budget** — core "hello world" < 15kb gzip, full core < 36kb gzip (raised from 30kb→32kb after keyboard-nav/a11y landed, 32kb→34kb after undo/redo landed, then 34kb→36kb after import/export facade wiring landed — all are Core-wide editor baseline UX, not plugin candidates). Non-core features are plugins.
