...
3. **Plugin for non-core** — MS Project, AI, custom calendar are separate plugins. A fully-composed core instance (base + `withIo` + `withRender` + `withInteraction`) is **23.71 KiB gzip, budget 24 KiB** — down from 34.9 KiB before the 2026-09 facade split, and the successor to the old "full core < 36kb" check (which measured `dist/index.js` as a plain file; code-splitting has since hollowed that file out). Enforced in CI via `pnpm size` (`packages/core/.size-limit.json`) — see `.claude/work/spec-bundle-size-ci.md` and CLAUDE.md golden rule 5 for the full per-capability table.
