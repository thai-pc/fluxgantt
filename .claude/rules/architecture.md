...
3. **Plugin for non-core** — MS Project, AI, custom calendar are separate plugins. A fully-composed core instance (base + `withIo` + `withRender` + `withInteraction`) is **23.82 KiB gzip, budget 24 KiB** — down from 34.9 KiB before the 2026-09 facade split, and the successor to the old "full core < 36kb" check (which measured `dist/index.js` as a plain file; code-splitting has since hollowed that file out). Enforced in CI via `pnpm size` (`packages/core/.size-limit.json`) — see `.claude/work/spec-bundle-size-ci.md` and CLAUDE.md golden rule 5 for the full per-capability table.
4. **Tree-shakable everything** — import only what you need, and this is now structural rather than aspirational: rendering, interaction and IO are opt-in mixins on separate subpath exports (`@fluxgantt/core/render`, `/interaction`, `/io`, `/theme`, `/responsive`), because class prototype methods never tree-shake and the old monolithic facade billed every consumer for all of them. "Hello world" (`createGantt()` alone) is **7.71 KiB gzip, budget 9 KiB** — down from 22.3 KiB pre-split. See CLAUDE.md golden rule 5 for every fixture.
