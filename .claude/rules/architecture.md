...
3. **Plugin for non-core** — MS Project, AI, custom calendar are separate plugins. Core bundle < 36kb gzip (raised from 30kb→32kb after keyboard-nav/a11y landed, 32kb→34kb after undo/redo landed, then 34kb→36kb after import/export facade wiring landed — all are Core-wide editor baseline UX, not plugin candidates; **now measured at ~34.9kb (CI, PR #39)** after the Canvas row-virtualization fix (#37)). This budget is enforced mechanically in CI via `pnpm size` (`packages/core/.size-limit.json`) — see `.claude/work/spec-bundle-size-ci.md`.
4. **Tree-shakable everything** — import only what you need. "Hello world" < 22kb gzip (re-baselined 2026-08; see CLAUDE.md golden rule 5 — the previous 15kb figure was never CI-verified, and the real, now-enforced measurement is ~21.3kb because the `Gantt` facade class doesn't tree-shake well yet; **now measured at ~22.3kb (CI, PR #39)** after #37 added `canvasViewportHeight` pass-through wiring to `gantt.ts` — only ~0.25kb of headroom left before this budget needs raising again).
